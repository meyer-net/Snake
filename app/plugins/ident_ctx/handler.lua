...
-- ctrl_pass = "{auth}/pass",                             -- 验证请求是否具备访问该URI的权限
...
-- 前台分布式微服务
-- 16a81a2e-bbd9-41a3-bd8f-5c8c4a487335 {"time":"2018-05-05 19:31:01","enable":true,"judge":{"conditions":[{"value":"^\/(.*)","operator":"match","type":"URI"}],"type":0},"id":"16a81a2e-bbd9-41a3-bd8f-5c8c4a487335","log":true,"name":"ALL","handle":{"log":true,"match_uri":"^\/api","ctrl_open":"{auth}\/api\/open","ctrl_pass":"{auth}\/api\/pass"},"default_host":"192.168.1.141","default_server":"http://192.168.1.141:9010","ident_field":"jwt", "ident_cookie": false,"ident_from":"{user}\/login","ident_destroy":"{user}\/logout","server_nodes":{"user":{"server_host":"192.168.1.141","server_url":"http://192.168.1.141:9002","match_uri":"\/user"},"www":{"server_host":"192.168.1.141","server_url":"http://192.168.1.141:9005","match_uri":"\/www"},"ims":{"server_host":"192.168.1.141","server_url":"http://192.168.1.141:9004","match_uri":"\/ims"},"order":{"server_host":"192.168.1.141","server_url":"http://192.168.1.141:9001","match_uri":"\/order"},"wallet":{"server_host":"192.168.1.141","server_url":"http://192.168.1.141:9003","match_uri":"\/wallet"},"auth":{"server_host":"192.168.1.141","server_url":"http://192.168.1.141:9008","match_uri":"\/auth","is_open":false}},"extractor":{"extractions":[{"type":"URI","name":"uri"}],"type":1}}

-- 后台单节点
-- 16a81a2e-bbd9-41a3-bd8f-5c8c4a487335 {"time":"2018-05-05 19:31:01","enable":true,"judge":{"conditions":[{"value":"^\/(.*)","operator":"match","type":"URI"}],"type":0},"id":"16a81a2e-bbd9-41a3-bd8f-5c8c4a487335","log":true,"name":"ALL","handle":{"log":true,"match_uri":"","default_host":"127.0.0.1","default_server":"http://127.0.0.1:5555", "ident_field":"jwt", "ident_cookie": false,"ident_from":"{api}\/manage\/users\/login","ident_destroy":"{api}\/manage\/users\/logout","ctrl_open":"{api}\/auth\/open\/?format=json","ctrl_pass":"{api}\/auth\/pass\/?format=json","server_nodes":{"api":{"server_host":"192.168.1.148","server_url":"http://192.168.1.148:8765","match_uri":"^\/api"}}},"extractor":{"extractions":[{"type":"URI","name":"uri"}],"type":1}}
-----------------------------------------------------------------------------------------------------------------
--[[
---> 统一函数指针
--]]
...
function handler:new(config, store)
    self.PRIORITY = 9999
    handler.super.new(self, config, store, "ident_ctx")
...
function handler:rewrite()
    ...
    local rule_pass_func = function (rule, variables)
        ...
        local load_api_exec = function (node, url, flow_args, ok_exec, false_exec, ignore_event_code)
            ...
            self:_log(rule, n_debug, s_format("[load-api-exec:%s:remote] url: %s, args: %s, resp: %s", node, url, log_args, c_json.encode(api_print)))

            local error_print = self:_get_print_from_ctrl(api_print, nil, ignore_event_code)
            if u_object.check(error_print) then
                self:_log(rule, n_info, s_format("[load-api-exec:%s:error] url: %s, args: %s, resp: %s", node, url, log_args, c_json.encode(error_print)))
            end
            ...
            if not error_print and ok ~= nil and (not ok or err) then
                error_print = ue_error.get_err(self._EVENT_CODE.program_err, err or "server is busying", api_print)
            end
        ...
        ---- 展开JWT信息
        local ident_string = ngx.var["cookie_"..rule_handle.ident_field]
        local ident_is_ok, ident_event_code, ident_secret, ident_payload = self._ident:load(ident_string, ngx.var.cookie_csrf_token)
            ...
            if from then
                ...
                local check_uri = function ( conf_uri )
                    ...
                    if new_uri ~= trim_conf_uri then
                        ...
                        self:_log(rule, n_info, s_format("[server-matched:{%s}] %s, conf_uri: %s, server_node: %s, server_uri: %s", match_uri, is_matched, conf_uri, micro_name, micro_uri))
                        return is_matched
                    end
                ...
                elseif check_uri(rule_handle.ctrl_open) or check_uri(rule_handle.ctrl_pass) then
                    case_error_exit(ue_error.get_err(self._EVENT_CODE.illegal))
                end
        ...
        if micro_uri then
            ...
            local load_micro_url = function ( var )
                ...
                if not server_node then
                    case_error_exit(ue_error.get_err(self._EVENT_CODE.program_err, s_format("can not find the server_node of '%s' in cache, please sure it exists", var)))
                end
            ...
            -- 如果是开放接口
            local ctrl_args = u_table.merge({ uri = request_uri, http_method = http_method }, { payload = ident_payload })
            load_api_exec("ctrl_open", load_micro_url(rule_handle.ctrl_open), ctrl_args, nil, function(open_print)
                if ident_is_ok then
                    ...
                    load_api_exec("ctrl_pass", load_micro_url(rule_handle.ctrl_pass), ctrl_args, nil, function(pass_print)
                        return pass_print.res, pass_print.msg, ue_error.get_err(self._EVENT_CODE.unauthorized)
                    end)
                else
                    -- 会话超时的情况
            ...
            if ident_is_ok then            
                local content_type_value = ngx.req.get_headers(0)["content-type"]
                ...
                if http_method == "get" then
                    ...
                    local trans_ok, trans_err = self._request:transform_post({
                        -- ident_id = ident_payload.jti,
                        payload = ident_payload
                    })

                    if trans_err then
                        case_error_exit(ue_error.get_err(self._EVENT_CODE.param_format_err, err))
                    end
                end
            end
            ...
            self:_log(rule, n_info, s_format("[server-rewrite:{%s}] server_host: %s, server_url: %s, rewrite_uri: %s, rewrite_uri_args: %s", request_uri, micro_host, micro_url, micro_uri, micro_uri_args))
        end
        ...
        if micro_url then
            ...
            if not u_object.check(micro_host) then -- host默认取请求的host
                n_var.upstream_host = ngx.var.host
            else 
            ...
            self:_log(rule, n_debug, s_format("[%s-match-micro-server:upstream] %s -> extractor_type: %s, upstream_host: %s, upstream_url: %s", self._name, rule.name, extractor_type, n_var.upstream_host, n_var.upstream_url))
        else
            self:_log(rule, n_info, s_format("[%s-match-micro-server:error] no upstream host or url. %s -> host: %s, uri: %s", self._name, rule.name, ngx.var.host, uri))
        end
    end
